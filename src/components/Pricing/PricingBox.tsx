    ...
    <div className="w-full px-4 md:w-1/2 lg:w-1/3">
      ...
      >
        {product.nickname === "Premium" && (
          <p className="absolute right-[-50px] top-[60px] inline-block -rotate-90 rounded-bl-md rounded-tl-md bg-rose-700 px-5 py-2 text-base font-medium text-white">
            New Curriculum 
          </p>
        )}
        <span className="text-rose-700 mb-5 block text-xl font-medium dark:text-white">
          {product.nickname}
        </span>
        <h2 className="mb-11 text-4xl font-semibold text-dark dark:text-white xl:text-[42px] xl:leading-[1.21]">
          ...
          <span className="text-xl font-medium">$ </span>
          <span className="-ml-1 -tracking-[2px]">
            {(product.unit_amount * 3).toLocaleString("en-US", {
              currency: "USD",
            })}
          </span>
          <span className="text-base font-normal text-body-color dark:text-dark-6">
        ...
        <div className="w-full">
          <button          
             className="inline-block rounded-md bg-rose-700 px-7 py-3 text-center text-base font-medium text-white transition duration-300 hover:bg-rose-700/90"
          >
          <a
